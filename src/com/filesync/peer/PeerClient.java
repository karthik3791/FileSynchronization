...
public class PeerClient implements Runnable {
	...
	private final Map<String, PeerClient> active_download_connections;
	...
	public PeerClient(String remoteip, Map<String, PeerClient> map) {
		...
		try {
			appender = new FileAppender(layout, SharedFolderDetails.logdir
					+ File.separatorChar + "Thread_downloading_from_"
					+ remote_ip + "_log", false);
		...
		try {
			...
			while (status) {
				...
				if (file_list_pkt.getflag().equals("FILELIST")) {
					...
					// 4. Compare the local file list with the remote file list.
					// Filter the files that needs to be pulled.
					String[] pull_list = getRequiredFiles(remote_file_list,
							local_file_list);
					...
					if (pull_list == null) {
						...
						// 5. Send the list of files which you need to pull from
						// the node.
						...
						if (keep_alive == true) {
							...
							if (server_status.getflag().equals("PUSHREADY")) {
								// Both server and client are ready to exchange
								// file.
	...
	public void close_connection() {
		// Remove the connection tracked in active_upload_connections and close
		// the socket.
		...
		if (file_write == true) {
			// Chance that the file was being written halfway when connection
			// was lost.
			try {
				fos.flush();
				fos.close();
				...
				if (tfile.exists()) {
					logMessage("File which was being pulled when connection was lost, is now being deleted. Status: "
							+ tfile.delete());
				}
	...
	// -- Compares local_file_list against remote_file_list and filters only the
	// required files --//
	...
	// Function takes in directory and returns the list of files in that
	// directory.
	...
	private void send_over_socket(String flag, String[] control_data)
			throws IOException {
	...
	private Packet read_from_socket() throws IOException,

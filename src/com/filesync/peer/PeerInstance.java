...
public class PeerInstance {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		//1. Initialize instance of Download Connections Manager and Upload Connections Manager
		...
		if (!syncdir.exists()){
			...
			if(result){    
				safePrintln("Synchronizing Directory : "+ SharedFolderDetails.directorypath+" created.");  
			}
		}
		...
		if (!logdir.exists()){
			...
			if(result){    
				safePrintln("Log Directory : "+ SharedFolderDetails.logdir+" created.");  
			}
		}
		...
		do{
			...
			if (choice ==1 ){
				...
				do{
					...
					try {
						...
						if(ip_address.isReachable(2000)){
							...
							// Check if either an outgoing or incoming connection already exists with this ip address.
							if(dm.checkDownloadConn(remoteip)==true || um.checkUploadConn(remoteip)==true){
				...
				do{
					...
					String remoteip;
					remoteip = br.readLine();
					safePrintln("Checking if a download/upload connection exists with this node at "+remoteip);

					if(dm.checkDownloadConn(remoteip) == false && um.checkUploadConn(remoteip)==false){
	...
	public static void print_all_download_connections( DownloadManager dm){
		...
		Map<String,PeerClient> active_download_connections = dm.getActiveDownloadConnections();

		safePrintln("********** DOWNLOAD CONNECTIONS    *****************");
		int no_active_connections =active_download_connections.keySet().size();		
		safePrintln("Totally "+no_active_connections+" download connections are active currently.");
	...
	public static void print_all_upload_connections( UploadManager um){
		...
		Map<String,PeerServer> active_upload_connections = um.getActiveUploadConnections();

		safePrintln("********** UPLOAD CONNECTIONS    *****************");
		int no_active_connections =active_upload_connections.keySet().size();		
		safePrintln("Totally "+no_active_connections+" upload connections are active currently.");
		if(no_active_connections > 0){
	...
	public static void connectToNode(String remoteip, DownloadManager dm){
		...
		PeerClient peerclient = new PeerClient(remoteip,dm.getActiveDownloadConnections());
